//
//  WriteSelectExpandableTagView.swift
//  MainHome
//
...
public struct WriteSelectExpandableTagView: View {
    @ObservedObject var viewModel: WriteSelectExpandableTagViewModel

    init(viewModel: WriteSelectExpandableTagViewModel) {
...
// 선택된 태그 뱃지 (헤더용)
private struct WriteTagBadge: View {
    let title: String
...
// 태그 칩 (선택용)
private struct WriteTagChip: View {
    let title: String
    let isSelected: Bool
    var action: (() -> Void)? = nil
...
#Preview {
    VStack {
        WriteSelectExpandableTagView(viewModel: WriteSelectExpandableTagViewModel(dbService: MainHomeMockDIContainer().makeDBImplementation()))
            .padding()
        Spacer()
    }
}
